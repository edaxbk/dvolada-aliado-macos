//
//  EditModifierController.swift
//  dVolada Aliado
//
//  Created by Miguel Ledezma on 27/04/21.
...
class EditModificadorControll : UIViewController,UITableViewDelegate  {
    ...
    var modifier : Modifier? {
        didSet{
            ...
            if let name = modifier?.title{
                nameModificadorText.text = name
            }
            radioController.buttonsArray = [optional,required]
            
            if (modifier?.isOptional == true){
                radioController.buttonArrayUpdated(buttonSelected: required)
            }else{
                radioController.buttonArrayUpdated(buttonSelected: optional)
            }
            if let array = modifier?.options{
                opcArray = array
            }
            
            
            if let limit = modifier?.option_selection_length{
                limitSelectedText.text = String(limit)
            }
    ...
    let titleLabel : UILabel = {
        ...
        label.text = "Editar tu Modificador"
    ...
    @objc func createModifierAction(sender: UIButton){
        ...
        if((nameModificadorText.text?.isEmpty)!){
            ...
            modifier?.title = nameModificadorText.text
        }
        
        if(optional.isSelected == true){
            print("is optional")
            modifier?.isOptional = false
        }
        ...
        if(opcArray.isEmpty){
            ...
            modifier?.options = opcArray
        }
        
        if((limitSelectedText.text?.isEmpty)!){
            modifier?.option_selection_length = 0
        }else{
            modifier?.option_selection_length = Int(limitSelectedText.text!)
        }
        
        ServerHelper.shared.putModifierFromStore(id: (modifier?._id)!, modifier: modifier!) { (succes) in
            print("ok",succes)
...
extension EditModificadorControll : UITableViewDataSource {
